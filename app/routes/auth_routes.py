...
@router.post("/logout")
def logout_user(token_data: LogoutRequest, db: Session = Depends(get_db)):
    stored_token = (
        db.query(RefreshToken)
        .filter(RefreshToken.token == token_data.refresh_token)
        .first()
...
def reset_password(request_data: ResetPasswordRequest, db: Session = Depends(get_db)):
    stored_token = (
        ...
        .filter(PasswordResetToken.token == request_data.reset_token)
        .first()
    )
    ...
    if stored_token.expires_at.replace(tzinfo=timezone.utc) < datetime.now(
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Reset token has expired"
        )
